...
import json #to load the JSON from the requests
import base64 #decode the texture values from the request
...
from time import sleep #uses the sleep function for the error screen 
...
class Software():
    def __init__(self):
        ...
        #size of the window and background color
        self.root.geometry("1000x525")
        self.root.config(bg = "#00838F")

        #tries to sets the icon at the top left to icon.ico
        try:
        ...
        #entry (text box that is only 1 line)
        self.user_entry = tk.Entry(self.root, width=60, font=("Calibri",18),fg="white", bg="#26C6DA", border=0)
        self.user_entry.pack()

        #FIND INFO button, executes getPlayerProperties() when clicked
        ...
        #download button, executes self.save_skin()
    ...
    def getPlayerProperties(self):
        try:
            ...
            #sets the image
            #self.skinPNG.image = self.tk_img
            
            #binds all of the changes
            ...
            #changes the UUID val to the error message
...
class PlayerProperties():
    def __init__(self,username):
        #GET the request from the Mojang API to get the player's uuid
        get_request = requests.get("https://api.mojang.com/users/profiles/minecraft/"+username)
        
        #load the Json request that was converted to text 
        loaded_req = json.loads(get_request.text)

        #get the player's uuid from the loaded request
        self.playerId = loaded_req["id"]

        #GET the request from the Mojang API to get the skin of the player
        ...
        #get the value from the loaded request, the value is in base64 so it also decodes it
...
if __name__ == '__main__':
    #if there's no connection errors
    try:
        Software()
    #if there's a connection error
    except:
        #prints this to the console
        print("Error: Check your network connection OR check if you're connected behind a firewall.\n") #2 new lines
        print("The Software will close soon")

        #5 seconds and the software closes
        sleep(5)
